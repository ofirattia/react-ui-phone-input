...
class PhoneInput extends React.Component {
	...
	handleFlagItemClick = (country, e) => {
		const {showOnlyDialCodes} = this.props;
	...
	getCountryDropdownList = () => {
		...
		const { disableDropdown, prefix, showOnlyDialCodes } = this.props
	...
	render() {
		...
		const { disableDropdown, renderStringAsFlag, isValid, defaultErrorMessage, specialLabel, showOnlyDialCodes, removeDialInput, inputLabel } = this.props;
		...
		const customCls = removeDialInput ? {'remove-dial-input': true} : {};
		...
		console.log(inputFlagClasses);
		return (
			<>
			{inputLabel ? (<div className='custom-input-label'>{inputLabel}</div>) : ''}
				...
				>
					{renderStringAsFlag ?
						...
						<div
							...
							className={`${selectedFlagClasses} ${showOnlyDialCodes ? 'show-only-dials' : ''}`}
						...
						>
							{showOnlyDialCodes ? (
								<>
								<div className="only-dials-dropdown">
